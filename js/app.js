(() => {
    ...
    const banerItem = document.querySelector(".baner-item");
    if (banerItem) {
        const banerStart = document.querySelector(".baner-item__start");
        const images = document.querySelectorAll(".baner-item__move");
        const banerMove = document.querySelectorAll(".baner-item__move");
        const clonedItem = banerStart.cloneNode(true);
        clonedItem.classList.add("second");
        banerItem.appendChild(clonedItem);
        let banerWidth = 0;
        for (let i = 0; i < images.length; i++) {
            banerWidth += +banerMove[i].getBoundingClientRect().width;
            const styles = window.getComputedStyle(banerMove[i]);
        ...
        function animationBanerText() {
            document.querySelectorAll(".baner-item__move").forEach((item => {
                item.animate([ {
                    transform: "translate(0, 0)"
                }, {
                    transform: "translate(-" + `${banerWidth + 4}` + "px, 0)"
